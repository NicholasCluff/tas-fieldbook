...
/**
 * Local debug version of the PDF processing logic
 ...
 */
async function extractPlansFromPDF(pdfDoc: PDFDocument, filename: string): Promise<ExtractedPlan[]> {
 ...
 */
function extractPlanReferencesFromFilename(filename: string): string[] {
 ...
 */
function createPlansFromReferences(references: string[], totalPages: number): ExtractedPlan[] {
 ...
 */
function analyzeDocumentStructure(totalPages: number, filename: string): ExtractedPlan[] {
  console.log(`\n=== DOCUMENT STRUCTURE ANALYSIS ===`);
  console.log(`No plan references found in filename, analyzing document structure`);
  console.log(`Total pages: ${totalPages}`);
  
  let numPlans: number;
  let pagesPerPlan: number;
 ...
 */
function createEvenlyDistributedPlans(numPlans: number, totalPages: number, filename: string): ExtractedPlan[] {
 ...
 */
function generatePlanReference(filename: string, index: number): string {
 ...
 * Main debug function
 */
async function debugProcessPDF(pdfFilePath: string) {
  try {
...
if (process.argv.length < 3) {
  console.log('Usage: node debug-pdf-processor.js <path-to-pdf-file>');
  console.log('');
  console.log('Example:');
  console.log('  node debug-pdf-processor.js ./test-documents/survey-432367-1.pdf');
  console.log('');
...
export { debugProcessPDF, extractPlansFromPDF };